...
async def health_check():
    ...
    return {"status": "healthy and working like a charm"}
...
async def predict(data: Data):
    ...
    if len(feature_array) != 30:
        raise HTTPException(status_code=400, detail="Please provide exactly 32 features.")

    prediction = model.predict([feature_array])
    final_verdict = pred_to_word(prediction[0])
    return {"data": {"prediction": final_verdict}}
